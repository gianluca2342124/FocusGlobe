...
struct UserProfile: Codable, Equatable {
    ...
    var name: String? = nil
    /// LEGACY. Onboarding asked this as free text, stored it here, and read it
    /// nowhere. The question is gone; the key stays so profiles saved by older
    /// builds keep decoding. Nothing writes it.
    var yearGoal: String? = nil
    /// LEGACY, for the same reason: an age band nothing ever read.
    var ageRange: String? = nil
    /// LEGACY, for the same reason: a free-text struggle nothing ever read. The
    /// first run still asks what breaks a pilot's focus, but that answer shapes
    /// the summary card in-flow and is deliberately not persisted — storing it
    /// again with no reader is exactly what made this field dead.
    var focusStruggle: String? = nil
    /// "What are you focusing on most?" — a `FocusPreset` title. LIVE: it is the
    /// Online flight category and the pre-selected focus token in flight setup.
    var focusStyle: String? = nil
    /// LEGACY. Onboarding no longer asks for Screen Time up front — the prompt
    /// belongs where the feature is used, not before the offer — and nothing
    /// read this. Kept for decoding only.
